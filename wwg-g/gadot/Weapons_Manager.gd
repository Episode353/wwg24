...
var weapon_stack: Array[String] = [] # An array of all weapons the player has
@onready var raycast_shoot: RayCast3D = $"../raycast_shoot"
var weapon_indicator: int = 0
var next_weapon: String = ""
@onready var ac_timer: Timer = $"../area_collision/Timer"

@onready var bullet_decal: PackedScene = preload("res://tscn/bullet_decal.tscn")

var weapon_list: Dictionary = {}
@export var _weapon_resources: Array[Dictionary] = []
@onready var raycast_wall: RayCast3D = $"../../../../raycast_wall"
@onready var area_collision: Area3D = $"../area_collision"
...
func _input(event: InputEvent) -> void:
	...
	if event.is_action_pressed("shoot") and weapon_raise == false:
		shoot()
...
func reset_all_ammo() -> void:
	...
	for weapon_name in weapon_stack:
		var weapon: Dictionary = weapon_list[weapon_name]
...
func Initalize(_start_weapons: Array[String]) -> void:
	if not is_multiplayer_authority(): return
	
	# Create a Dictionary to refer to our weapons
	for weapon in _weapon_resources:
		print("Adding weapon: ", weapon.weapon_name)  # Debugging statement
		weapon_list[weapon.weapon_name] = weapon

	print("Weapon list: ", weapon_list)  # Debugging statement to check the contents of weapon_list

	for i in _start_weapons:
		weapon_stack.push_back(i) # Add our start weapons

	print("Weapon stack: ", weapon_stack)  # Debugging statement to check the contents of weapon_stack
...
func change_weapon(weapon_name: String) -> void:
	...
	var weapon_range: float = current_weapon.weapon_range
...
func raycast_shoot_procc() -> void:
	var hit_object: Object = raycast_shoot.get_collider()
	# Place the Bullet Decal
	var col_nor: Vector3 = raycast_shoot.get_collision_normal()
	var col_point: Vector3 = raycast_shoot.get_collision_point()
	var b: Node3D = bullet_decal.instantiate()
...
func area_collision_procc(self_id: int) -> void:
	var ac_enim: Array = area_collision.get_overlapping_bodies()
	print("Self ID: ", self_id) # Print self ID for debugging
	for e: Node in ac_enim:
		var entity: Node = e
		var e_id: int = entity.get_instance_id() # Get the unique ID of each entity
		...
		if entity.is_in_group("players"):
			...
			if entity.name != str(self_id): # Compare with the player ID passed to the function
				print("Damaging player: Entity ID: ", e_id) # Debug print
				entity.rpc("receive_damage", current_weapon.damage)
			else:
	...
	if current_weapon.disable_ammo == false:
		if current_weapon.current_ammo == current_weapon.mag_ammo:
			...
			if current_weapon.reserve_ammo != 0:
				...
				var reload_ammount: int = min(current_weapon.mag_ammo - current_weapon.current_ammo, current_weapon.mag_ammo, current_weapon.reserve_ammo)
